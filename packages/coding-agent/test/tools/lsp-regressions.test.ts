...
describe("lsp regressions", () => {
	...
	describe("reload cancellation and truthful teardown (#6369)", () => {
		...
		it("falls back when method-not-found is signalled by JSON-RPC code -32601 with a non-matching message (#6403)", async () => {
			...
			try {
				const server = installFakeLsp((message, srv) => {
					if (message.method === "initialize") {
						...
						// -32601 but a message the substring check would NOT catch.
				...
				const config: ServerConfig = { command: "fake-reload-code", fileTypes: [".ts"], rootMarkers: [] };
				vi.spyOn(lspConfig, "loadConfig").mockReturnValue({ servers: { fake: config }, idleTimeoutMs: undefined });
